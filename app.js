...
if(!grid)return;
grid.innerHTML=SERVICES.map((s,i)=>`<article class="service-card"><div class="service-number">0${i+1}</div>${s[3].length?`<div class="service-images">${s[3].map(x=>`<img src="${x}" alt="${s[0]}">`).join("")}</div>`:""}<div class="service-content"><h3>${s[0]}</h3><p>${s[2]}</p><div class="service-bottom"><strong>${money(s[1])}</strong><button class="primary small" onclick="openBooking(${i})">Agendar</button></div></div></article>`).join("")
}

function renderGallery(){
if(!galleryGrid)return;
const all=SERVICES.flatMap(s=>s[3].map(img=>({img,name:s[0]})));
galleryGrid.innerHTML=all.map(x=>`<figure><img src="${x.img}" alt="${x.name}"><figcaption>${x.name}</figcaption></figure>`).join("")
}
...
window.open(`https://wa.me/5531972084333?text=${encodeURIComponent(`Olá, Débora! Gostaria de agendar:\n${s}\nData: ${d}\nHorário: ${t}`)}`,"_blank")
}
